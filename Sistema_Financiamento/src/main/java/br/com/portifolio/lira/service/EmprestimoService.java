...
public class EmprestimoService {
    ...
    public static void imprimirEmprestimosByPessoa(String id) {
        Pessoa pessoa = PessoaService.buscarPessoa(id);
        if (pessoa != null){
            List<Emprestimo> emprestimoList = emprestimoDao.findByPessoa(pessoa);
            if (emprestimoList.size() > 0) {
                emprestimoList.forEach(System.out::println);
            }
            else {
                System.out.println("Cliente sem contratos");
            }
        }
    }
